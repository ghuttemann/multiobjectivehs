    ...
    try {
        if (argc != 8)
            throw invalid_argument("Use: <programFile> <hsConfigFile> <problem> "
                    "<algorithm> <calculateMetrics> <doSampling> <modalMode> <nObjectives>");
        
        if (atoi(argv[7]) == 0)
            throw invalid_argument("Expected a number of objectives greater than zero");
        ...
        if (strcasecmp(argv[2], "zdt1") == 0)
            ...
            problem = new CEC09UnProb5(strcat(path, "/cec09/cec09unprob5.txt"),
                    hsParams, new RealVariableBuilder());
        else if (strcasecmp(argv[2], "dtlz1") == 0)
            problem = new DTLZ1(atoi(argv[7]), strcat(path, "/dtlz/dtlz1.txt"),
                    hsParams, new RealVariableBuilder());
        else if (strcasecmp(argv[2], "dtlz2") == 0)
            problem = new DTLZ2(atoi(argv[7]), strcat(path, "/dtlz/dtlz2.txt"),
                    hsParams, new RealVariableBuilder());
        else
            throw invalid_argument("Invalid problem.\n"
                    "Options(regex): zdt[123456] | cec09unprob[12345] | dtlz[12]");
        ...
        if (strcasecmp(argv[3], "hs1") == 0)
            algorithm = new HarmonySearch1(problem, hsParams, memBuild,
                memInit, memCons, pitchAdj, memUp, parRank, calculateMetrics, doSampling, true);
        else if (strcasecmp(argv[3], "hs2") == 0)
            algorithm = new HarmonySearch2(problem, hsParams, memBuild,
                memInit, memCons, pitchAdj, memUp, parRank, calculateMetrics, doSampling);
        else if (strcasecmp(argv[3], "hs3") == 0)
            algorithm = new HarmonySearch3(problem, hsParams, memBuild,
                memInit, memCons, pitchAdj, memUp, parRank, calculateMetrics, doSampling);
        else if (strcasecmp(argv[3], "random") == 0)
            algorithm = new RandomAlgorithm(problem, hsParams, calculateMetrics, doSampling);
        else
            throw invalid_argument("Invalid algorithm.\n"
                    "Options(regex): hs[123] | random");
